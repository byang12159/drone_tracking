...
class Simulation():
    def __init__(self, totalcount=1000):
        ...
        self.client1.takeoffAsync(self.lead).join()


        self.client1.enableApiControl(True,self.chase)
        self.client1.armDisarm(True, self.chase)
        self.client1.takeoffAsync(self.chase).join()


        # self.client1.moveToPositionAsync(-10, 10, -10, 5, vehicle_name=self.lead)
        # self.client2.moveToPositionAsync(-10, 10, -10, 5, vehicle_name=self.chase).join()


        print("Drones takeoff")
    ...
    def get_NED(self, vehicle_name, client, get_quaternion=False):
        ...
        GT_states = self.client1.getMultirotorState(self.lead).kinematics_estimated
    ...
    def get_Global(self, vehicle_name, get_quaternion=False):
        if vehicle_name == "Drone_L":
            drone = self.lead
            client = self.client1
        else:
            drone = self.chase
        ...
        if get_quaternion==False:
            drone_pose = client.simGetVehiclePose(drone).position
            return [drone_pose.x_val, drone_pose.y_val,drone_pose.z_val]
        else:
            drone_pose = client.simGetVehiclePose(drone).orientation
            return [drone_pose.w_val, drone_pose.x_val, drone_pose.y_val,drone_pose.z_val]
    ...
    def move_lead(self):
        ...
        try:
            while True:
                ...
                # client.moveByVelocityBodyFrameAsync(0,2,0, self.timestep, vehicle_name = self.lead)
                client.moveByVelocityBodyFrameAsync(0,efforty,0, self.timestep, vehicle_name = self.lead)
                # client.moveToPositionAsync(100,0,40,vehicle_name=self.lead)
                print("effprt ",efforty)
                curr_time = time.time()
        ...
        #     client.moveOnPathAsync(waypoints, 1, 60 ,airsim.DrivetrainType.MaxDegreeOfFreedom, airsim.YawMode(False,0), 1, 1, vehicle_name = self.lead)                
            ...
            traceback.print_exc()
            event.set()  # Signal the error event




    def move_chase(self, event, thread_id):
        global count
        client = self.client1
        ...
        try:
            while True:
                ...
                print("POS DIFF Y: " , client.simGetObjectPose(self.lead).position.y_val - client.simGetObjectPose(self.chase).position.y_val)
    ...
    def processing(self):
        ...
        self.variance_history = np.array(self.variance_history)
        ...
        times = np.arange(0,self.variance_history.shape[0])*self.timestep
        varx.plot(times, self.variance_history[:,0], label = "Var x")
        varx.legend()
        vary.plot(times, self.variance_history[:,1], label = "Var y")    
        vary.legend()
        varz.plot(times, self.variance_history[:,2], label = "Var z")
        varz.legend()
        plt.show()
...
if __name__ == "__main__":
    ...
    sim = Simulation()
    print("1")
    # sim.client1.moveToPositionAsync(-10, 10, -10, 5, vehicle_name=sim.lead).join()
    print("2")
    # error_event = threading.Event()
    # threadL = threading.Thread(target=sim.move_lead, args=(error_event, 1), name='Thread lead')
    # threadC = threading.Thread(target=sim.move_chase, args=(error_event, 1), name='Thread Chase')
    # global retX
    # retX = []
    # global retY
    # retY = []
    # for i in range(100):
    #     sim.client1.moveByVelocityBodyFrameAsync(0,2,0, sim.timestep, vehicle_name = sim.lead)
    sim.move_lead()
    print("2")
    ...
    # while threadL.is_alive() or threadC.is_alive():
    #     if error_event.is_set():
    #         print("Error detected, stopping all threads.")
    #         break
    #     time.sleep(0.1)
