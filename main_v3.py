...
class simulation():
    ...
    def move_lead(self):
        ...
        if mode == "simple_vel":
            while True:
                ...
                client.moveByVelocityBodyFrameAsync(0,1,0, self.timestep, vehicle_name = self.lead)
                curr_time = time.time()
    ...
    def processing(self):
        ...
        posx.plot(times, self.velocity_GT[:,0], label = "GT Vel x")
        posx.legend() 
        posy.plot(times, self.velocity_GT[:,1], label = "GT Vel y")
        posy.legend()
        posz.plot(times, self.velocity_GT[:,2], label = "GT Vel z")
        posz.legend()
...
if __name__ == "__main__":
    ...
    for run in range(runs):
        ...
        try:
            ...
            sim.variance_history.insert(0,[f"Run #{run}"])
            with open(csv_file, 'a', newline='') as csvfile:
                csvwriter = csv.writer(csvfile)
                csvwriter.writerows(sim.variance_history)
